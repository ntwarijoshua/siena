...
func testUserToOneProfileUsingProfile(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := local.Profile().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadProfile(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	local.R.Profile = nil
	if err = local.L.LoadProfile(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Profile == nil {
		t.Error("struct should have been eager loaded")
...
func testUserToOneSetOpProfileUsingProfile(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Email`: `character varying`, `Password`: `character varying`, `Confirmed`: `boolean`, `ProfileID`: `integer`, `RoleID`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `Deleted`: `boolean`}
	_           = bytes.MinRead
)
