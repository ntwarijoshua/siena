...
var RoleWhere = struct {
	...
	Deleted:   whereHelperbool{field: "\"roles\".\"deleted\""},
}
...
var (
	roleAllColumns            = []string{"id", "name", "slug", "created_at", "updated_at", "deleted"}
	roleColumnsWithoutDefault = []string{"name", "slug", "created_at"}
	roleColumnsWithDefault    = []string{"id", "updated_at", "deleted"}
...
func (roleL) LoadUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`users`), qm.WhereIn(`users.role_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
